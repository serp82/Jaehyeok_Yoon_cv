...
전파 자원이 공유되는 환경에서는 레이더·통신·식별 신호가 같은 시간과 주파수 대역을 사용하면서 수신 신호가 서로 겹칠 수 있습니다. 이런 환경은 특정 신호만 깨끗하게 관측할 수 있다는 가정이 성립하지 않기 때문에, 실제 무선 수신기에서 신호 종류와 점유 상태를 함께 파악해야 합니다.

공존 신호를 안정적으로 식별하지 못하면 주파수 공유 판단과 후단 신호처리의 신뢰도가 떨어집니다. 따라서 여러 신호가 섞인 입력을 실시간으로 분석하고, 인식 결과를 자동 평가로 연결하는 시스템이 필요합니다.
